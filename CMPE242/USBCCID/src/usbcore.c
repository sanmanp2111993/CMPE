...
#endif  /* USB_MSC */
#if USB_CCID
              if (SetupPacket.wIndex.WB.L == USB_CCID_IF_NUM) {           /* IF number correct? */
                switch (SetupPacket.bRequest) {
                  case CCID_REQUEST_ABORT:
                    CCID_SlotAbort = SetupPacket.wValue.WB.L;
                    CCID_SeqAbort = SetupPacket.wValue.WB.H;
                    goto setup_class_ok;
                    break;
