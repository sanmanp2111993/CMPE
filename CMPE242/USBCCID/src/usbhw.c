...
volatile uint32_t USBConfigInfo;    /* For debugging use. */
...
void USBIOClkConfig( void )
{
    ...
    /* Be careful that p0.1 is used as remote wakeup source or for some other purpose. */
...
void USB_Init (void) {
    ...
    /* Check IP3511 USB configuration for debugging. */
    // USBConfigInfo = LPC_USB->CONFIGURATION;
#if LPM_SUPPORT
    ...
    /* Internal PLL or external clock is used for USB. */
    // LPC_USB->INTERNALPLL = 0;
...
uint32_t USB_ReadSetupEP (uint32_t EPNum, uint8_t *pData) {
    ...
    for(n = 0; n < (cnt + 3) / 4; n++)    {
#if defined(__GNUC__)
        (( unaligned_uint32 *)pData)->value = (( unaligned_uint32 *)dataptr)->value;
#else
...
uint32_t USB_WriteEP (uint32_t EPNum, uint8_t *pData, uint32_t cnt) {
    ...
    *addr |= (cnt<<16)|(uint16_t)((EPList[index].buf_ptr)>>6);
    ...
    for(n = 0; n < (cnt + 3) / 4; n++)    {
#if defined(__GNUC__)
        (( unaligned_uint32 *)dataptr)->value = (( unaligned_uint32 *)pData)->value;
#else
